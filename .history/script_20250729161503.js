...
function toggleAudio(songName) {
  ...
  const bandName = card.querySelector('.band-name');
  const albumTitle = card.querySelector('.album-title');

  // Remove animation from all other songs
  document.querySelectorAll('.band-name').forEach(el => el.classList.remove('move-words'));
  document.querySelectorAll('.album-title').forEach(el => el.classList.remove('move-words'));

  if (audio.paused || audio.ended) {
    playAudio(songName);
    playBtn.classList.remove("fa-play");
    playBtn.classList.add("fa-pause");
    bandName.classList.add("move-words");
    albumTitle.classList.add("move-words");
  } else {
    pauseAudio(songName);
    playBtn.classList.remove("fa-pause");
    playBtn.classList.add("fa-play");
    bandName.classList.remove("move-words");
    albumTitle.classList.remove("move-words");
  }

  // Keep play/pause icon in sync with audio events
  audio.onpause = () => {
    playBtn.classList.remove("fa-pause");
    playBtn.classList.add("fa-play");
    bandName.classList.remove("move-words");
